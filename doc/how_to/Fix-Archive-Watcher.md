...
- stop the archive watcher service and disable it
...
- map a network drive to `data$` on your instrument e.g. for GEM you would map a network drive to `\\ndxgem\data$`. You need to give the `ndxgem\spudulike` username and password for this drvie to connect
- run `archive_water.exe` in `c:\Program Files (x86)\STFC ISIS Facility\ISIS Archive Watcher` interactively, optionally create a shortcut in the user logion startup menu 
