...
Spectra plots can be displayed in either ``counts`` or ``counts/μs`` mode. In ``counts`` mode, the plot shows the raw sum of counts in the given bin. If bin widths are not uniform, the plot of ``counts`` will have discontinuities at time-binning boundaries. In ``counts/μs`` mode, the displayed number is normalised by bin width (this is usually preferred for visualisation).
